...
namespace HunterCombatMR.Models.UI.AnimationTimeline
{
        ...
        : UIElement
    {
        ...
        public AnimationBuilder Animation { get; set; }
        public AnimationDef Animator { get; set; }
        ...
        {
            Animator = new AnimationDef(Animation.Build());

            for (int f = 0; f < Animation.Keyframes.Count(); f++)
            {
                var element = new TimelineKeyFrameGroup(this, 
                    (Animation.Layers.Any(x => x.HasLayerData(f))) ? FrameType.Keyframe : FrameType.Empty, 
                    f, 
                    Scale, 
                    Animator.Keyframes[f].Frames);
        ...
        public void SetAnimation(AnimationBuilder animation)
        {
            FrameList.Clear();
            Animation = animation;
